...
function initMap() {
    ...
    var getAirplanes = function() {
        if(radarlivre_updater.doBeginConnection("airplane")) {
            ...
            radarlivre_api.doGetAirplaneInfos(
                null, mapsBounds, 
                function(data) {
                    radarlivre_updater.doEndConnection("airplane", data, "airplane");
                }, 
                function(error) {
                    radarlivre_updater.doCancelConnection("airplane");
    ...
    var getContrib = function() {
        if(radarlivre_updater.doBeginConnection("contrib")) {
            log("Begin get contribs...");
            radarlivre_api.doGetContribs(
                null, 
                function(data) {
                    radarlivre_updater.doEndConnection("contrib", data);
                }, 
                function(error) {
                    radarlivre_updater.doCancelConnection("contrib");
    ...
    var getRoute = function() {
        ...
        if(marker) {
            if(radarlivre_updater.doBeginConnection("route")) {
                log("Begin get route to: " + marker.id);
                radarlivre_api.doGetAirplaneRoute(
                    marker.id, 
                    null, 
                    function(data) {
                        radarlivre_updater.doEndConnection("route", data);
                    }, 
                    function(error) {
                        radarlivre_updater.doCancelConnection("route");
    ...
    radarlivre_updater.doSetOnObjectCreatedListener(function(objects, connectionType, conn) {
        ...
        } else if(connectionType == "contrib") {
            for(o of objects) {
                maps_api.doSetMarker({
                    id: o.id, 
                    dataType: connectionType, 
